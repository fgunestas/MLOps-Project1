...
def lead_model(model_path:str):
    return joblib.load(model_path)

def predict(model:XGBClassifier,X_test:pd.DataFrame):
    return model.predict(X_test)

def log_param(model:XGBClassifier,features:list):
...
def evaluate(config:DictConfig):
    mlflow.set_tracting_uri(config.mlflow_tracking_ui)
    ...
    with mlflow.start_run():
        ...
        log_param(model,config.process.features)
